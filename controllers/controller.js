...
router.get('/scrape', function (req, res) {
    request('http://foxnews.com', function (error, response, html) {
        ...
        $('article .inner').each(function (i, element) {
            ...
            result.link = 'http://foxnews.com/' + $(this).children('header').children('h2').children('a').attr('href').trim();
                ...
                if (titlesArray.indexOf(result.title) == -1) {
                    ...
                    Article.count({ title: result.title }, function (err, test) {
                        ...
                        if (test == 0) {
                            ...
                            console.log('Already in Database. Not saved to DB.')
                        }
                    });
                }
                else {
                    console.log('Article already saved, Not Saved to DB.')
                }
            }
            else {
